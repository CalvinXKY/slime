#!/bin/bash
# Slime P2P weight update long run: Qwen3-4B, TP=4, 50 rollout steps.
set -ex
...
LOG_FILE="${LOG_ROOT}/slime_p2p_50step_tp4_${TS}.log"
ERR_FILE="${LOG_ROOT}/slime_p2p_50step_tp4_${TS}.err"

HF_CKPT="${HF_CKPT:-/data/nfs_87/xky/models/Qwen3-4B}"
REF_LOAD="${REF_LOAD:-/data/nfs_87/xky/models/Qwen3-4B_torch_dist}"
SLIME_SAVE="${SLIME_SAVE:-/data/nfs_87/xky/models/Qwen3-4B_slime_p2p_50step}"
PROMPT_DATA="${PROMPT_DATA:-/data/nfs_87/wx/data/slime/gsm8k/train.parquet}"

echo "=== P2P 50-step | LOG_FILE=${LOG_FILE} ==="
...
EXIT_CODE=$?
echo "P2P 50-step finished at $(date -Is) exit_code=${EXIT_CODE}"
