#!/bin/bash
# NC baseline (all_gather + broadcast): Qwen3-4B TP=4, 50 rollout steps, logs on NFS.
set -ex
...
LOG_FILE="${LOG_ROOT}/slime_nc_baseline_50step_tp4_${TS}.log"
ERR_FILE="${LOG_ROOT}/slime_nc_baseline_50step_tp4_${TS}.err"

HF_CKPT="${HF_CKPT:-/data/nfs_87/xky/models/Qwen3-4B}"
REF_LOAD="${REF_LOAD:-/data/nfs_87/xky/models/Qwen3-4B_torch_dist}"
SLIME_SAVE="${SLIME_SAVE:-/data/nfs_87/xky/models/Qwen3-4B_slime_nc_baseline_50step}"
PROMPT_DATA="${PROMPT_DATA:-/data/nfs_87/wx/data/slime/gsm8k/train.parquet}"

echo "=== NC Baseline 50-step (unmodified slime) | LOG_FILE=${LOG_FILE} ==="
...
EXIT_CODE=$?
echo "NC Baseline 50-step finished at $(date -Is) exit_code=${EXIT_CODE}"
