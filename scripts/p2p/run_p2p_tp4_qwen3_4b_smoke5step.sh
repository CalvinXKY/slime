#!/bin/bash
# Slime P2P weight update smoke test: Qwen3-4B, TP=4, 5 rollout steps.
set -ex
...
# Leave unset in production; set to 1 only when debugging NCCL hangs.
...
LOG_FILE="${LOG_ROOT}/slime_p2p_smoke5step_${TS}.log"
ERR_FILE="${LOG_ROOT}/slime_p2p_smoke5step_${TS}.err"

HF_CKPT="${HF_CKPT:-/data/nfs_87/xky/models/Qwen3-4B}"
REF_LOAD="${REF_LOAD:-/data/nfs_87/xky/models/Qwen3-4B_torch_dist}"
SLIME_SAVE="${SLIME_SAVE:-/data/nfs_87/xky/models/Qwen3-4B_slime_p2p_smoke5}"
PROMPT_DATA="${PROMPT_DATA:-/data/nfs_87/wx/data/slime/gsm8k/train.parquet}"

echo "=== P2P smoke 5-step | LOG_FILE=${LOG_FILE} ==="
