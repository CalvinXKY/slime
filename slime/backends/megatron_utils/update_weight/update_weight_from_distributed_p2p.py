"""
Shard-level P2P weight update for non-colocate Slime + SGLang.

Purpose
-------
Sync Megatron training weights to SGLang rollout engines without the default
all_gather + NCCL broadcast path. Each training TP rank converts its own shard
to HF layout and sends it directly to the matching inference TP rank via
``dist.send`` / ``dist.recv``.

Enable with ``--use-p2p-weight-update`` (requires ``--update-weight-mode=full``
and non-colocate mode). When Megatron TP != SGLang TP, Slime falls back to
``UpdateWeightFromDistributed`` (NCCL broadcast) automatically; see
``p2p_tp_sizes_match`` in ``common.py``.

Prerequisites
-------------
- SGLang side: apply ``docker/patch/latest/sglang_p2p.patch`` (presharded recv,
  ``tp_tensor_counts``, NCCL barrier sync, ``device_id`` for PyTorch 2.11+).
- Megatron TP must equal SGLang TP for every rollout engine
  (``tensor_model_parallel_size == rollout_num_gpus_per_engine // sglang_pp_size``).
- Multiple rollout engines (replicas) are supported: the same shard is sent to
  each engine's corresponding TP rank.

Update flow (per step)
----------------------
1. Vocab params (embed / lm_head): small TP all_gather, strip Megatron padding,
   slice to SGLang-aligned shards (Megatron and SGLang partition vocab differently).
2. Other params: shard-level Megatron→HF conversion, no all_gather.
3. Per bucket: ``all_gather_object`` metadata → rank-0 HTTP notify → NCCL barrier
   → parallel ``dist.send`` to all engines → ``ray.get`` for load completion.

See ``scripts/p2p/`` for smoke / long-run launch scripts and SGLang patch helper.
"""
...
from .common import all_gather_param, get_sglang_tensor_parallel_size, named_params_and_buffers, p2p_tp_sizes_match
...
class UpdateWeightFromDistributedP2P:
    ...
    def _resolve_updater(
        self, engine_gpu_counts: Sequence[int] | None = None
    ) -> UpdateWeightFromDistributed | None:
        """Return broadcast fallback when TP sizes mismatch; None to use P2P."""
        if self._use_p2p is False:
            return self._get_fallback()
        if self._use_p2p is None:
            if p2p_tp_sizes_match(self.args, engine_gpu_counts):
                self._use_p2p = True
                return None
            self._use_p2p = False
            if self.tp_rank == 0 and self._is_dp0:
                counts = engine_gpu_counts or [self.args.rollout_num_gpus_per_engine]
                sglang_tps = [get_sglang_tensor_parallel_size(self.args, c) for c in counts]
                print(
                    "[P2P] Megatron TP "
                    f"({self.args.tensor_model_parallel_size}) != SGLang TP(s) {sglang_tps}; "
    ...
    def update_weights(self) -> None:
        ...
        self._log_timing("Phase1 vocab (all_gather+convert+slice)",
        ...
        self._log_timing("Phase2 non-vocab (convert_shard+send)",
        ...
        self._log_timing("Phase3 expert (direct send)",
                         t_phase3_end - t_phase3_start,
                         f" [{expert_count} params]")
